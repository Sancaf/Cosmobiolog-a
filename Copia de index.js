...
function getOutcome() {
	...
	let monthsFromBirth = actualDate.getMonth() - birthmonth.value + 1
	let age = actualDate.getFullYear() - birthyear.value
	let andWhatAboutTheDay = actualDate.getDate() - birthday.value
	...
	if (
		monthsFromBirth < 0 ||
		(monthsFromBirth === 0 && actualDate.getDate() < birthday)
	) {
		age--
		monthsFromBirth += 12
	}

	if((actualDate.getDate() < birthday) >= 0){
		monthsFromBirth += 1
	}
